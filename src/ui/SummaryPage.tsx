...
export function SummaryPage() {
  ...
  const silentHazards = events.filter(event => event.kind === "hazard" && event.fromState === "SILENT");
  ...
  return (
    <main className="page">
      ...
      <section className="stats">
        ...
        <Stat label="Silent → Hazard" value={summary.silentToHazardEntries} note="Direct Silent → Hazard entries" tone="hazard" />
        <Stat label="Avg Silent → Hazard reaction" value={avgSilentHazardLag == null ? "—" : `${(avgSilentHazardLag/1000).toFixed(2)} s`} note={`${silentHazardLags.length} correlated brake-release → Silent/Hazard event${silentHazardLags.length===1?"":"s"} · expected up to 5 s`} tone="hazard" />
