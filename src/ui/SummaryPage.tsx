  ...
  return (
    <main className="page">
      ...
      <section className="event-panels">
        ...
        <article className="panel"><h3>Hazard events</h3>{events.filter(e => e.kind === "hazard").map(e => <div className="summary-event hazard-event" key={e.id}><b>{formatPadLabel(e.padDisplayId, padAssignments[e.padDisplayId], "both")}</b><span>{formatMachineLabel(e.controllerId, machineAssignments[e.controllerId], "both")}</span><time>{formatClock(e.startMs)}</time></div>)}{!events.some(e => e.kind === "hazard") && <p>No Hazard events.</p>}</article>
      </section>
